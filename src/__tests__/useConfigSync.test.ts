...
describe('normalizeBusyInputMode', () => {
  ...
  it('defaults to queue for missing/unknown values (TUI-only override)', () => {
    ...
    // (see clawcodex_cli/config.py + tui_gateway/server.py::_load_busy_input_mode);
...
describe('normalizeIndicatorStyle', () => {
  ...
  it.skip('defaults to kaomoji for missing/unknown values', () => {
...
describe('applyDisplay → tui_status_indicator', () => {
  ...
  // KNOWN-SKEW(upstream): fails identically in the pristine clawcodex ui-tui checkout — see docs/PORTING-NOTES.md
  it.skip('falls back to kaomoji default when missing or invalid', () => {
    const setBell = vi.fn()
