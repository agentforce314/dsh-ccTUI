import { Box, type ScrollBoxHandle, stringWidth, Text } from '@clawcodex/ink'
...
const EMOJI_FRAMES = ['✦ ', '🌀', '🤔', '✨', '🍵', '🔮']
const ASCII_FRAMES = ['|', '/', '-', '\\']
// Claude Code's breathing "star" spinner: ramps · → ✽ then back down so the
// glyph appears to pulse/breathe while the agent works.
const STAR_FRAMES = ['·', '✢', '✳', '✶', '✻', '✽', '✻', '✶', '✳', '✢']
...
const renderIndicator = (style: IndicatorStyle, tick: number): IndicatorRender => {
  ...
  if (style === 'star') {
    return {
      frame: STAR_FRAMES[tick % STAR_FRAMES.length] ?? '✻',
      intervalMs: 120,
      showVerb: true
    }
  }
...
export function StatusRule({
  ...
  // Dev-only readout (CLAWCODEX_DEV_CREDITS). The server omits the key entirely unless the
  ...
  // Dev-gated readout (CLAWCODEX_DEV_CREDITS), lowest priority,
